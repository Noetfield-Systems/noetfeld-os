#!/usr/bin/env bash
# Dispatch Phase-1 GHA secondary witnesses and wait for success (bootstrap before cron).
set -euo pipefail
...
for wf in "${workflows[@]}"; do
  log "dispatch $wf"
  url="$(gh workflow run "$wf" --repo "$REPO" 2>&1 | tail -1)"
  run_id="${url##*/}"
  if [[ -z "$run_id" || "$run_id" == "$url" ]]; then
    sleep 3
...
if [[ "$fail" -ne 0 ]]; then
  log "witness phase1 smoke: FAIL"
  exit 1
fi

log "witness phase1 smoke: PASS (${#workflows[@]} workflows)"
exit 0
