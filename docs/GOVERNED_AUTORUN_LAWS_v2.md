...
Same inputs → same transitions → same receipts. Idempotency keys, single writer + CAS, IDs from actuals, advance = f(acks), LLM as proposal only, verify as pure function. See `deterministic-loops.md` (D1–D8) and legal transition table.

---

## Deterministic loop core (D1–D8) — summary

| Rule | Law |
|------|-----|
| D1 | Idempotency keys on every side effect |
| D2 | Single writer per state cell; CAS advance |
| D3 | IDs from max(actual) + 1, never inference |
| D4 | Advance := execute_ok AND validate_ok AND sink_acked |
| D5 | Event log is truth; state = fold(events) |
| D6 | Randomness/time quarantined to scheduling edge |
| D7 | LLM output is proposal, not transition |
| D8 | Verify is pure function; runner divergence = escalation |

**CI determinism gate (target):** replay test, idempotency test, CAS test, illegal-transition fuzz.

---

## Parallel orchestration, cycle anatomy, Kaizen, audit pass

See `.cursor/skills/governed-autorun/SKILL.md` for full parallel orchestration, cycle anatomy, Kaizen ROI queue, and adversarial audit checklist.
...
3. Scheduled trigger proven: 2+ consecutive `schedule`-event success runs on the **same workflow** (factory autorun: two consecutive `noos-factory-autorun.yml` runs ~10m apart)
