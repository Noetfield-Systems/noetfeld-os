# nf-mkt-0021 — VerifyWise · Run history & proof
...
From https://verifywise.ai · https://verifywise.ai/pricing document VerifyWise run/history UX: Registry → risk assessment → policy enforcement → audit export — map to `TLE evaluate + drift export + copilot pilot e2e`

## Implementation extraction

`VerifyWise · Run history & proof` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `nf-mkt-0021`
