# nf-mkt-0031 — VerifyWise · Onboarding & PLG
...
Document VerifyWise onboarding path (Open source → free tier → enterprise): who runs (Self-hosted or managed cloud; source-available) → first value moment

## Implementation extraction

`VerifyWise · Onboarding & PLG` → what buyer sees at vendor → what we ship on disk with receipt.
...
1. `status: done` in REGISTRY.json for `nf-mkt-0031`
