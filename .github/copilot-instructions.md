...
- Treat `Noetfield-Systems/noetfeld-os` as the canonical active repo slug for this repo.
- Treat `kazemnezhadsina144[-]dot` as a **forbidden active-config marker**.
- If the pre-migration personal slug (see `noetfield-org/FORBIDDEN_MARKERS.txt`) appears in active config, repo URLs, dispatch defaults, owner handles, or machine routing config, migrate it or flag it explicitly.
